...
begin
  ...
  -- An active BU membership overrides the company role/permission preset for the selected BU.
  if v_bu is not null then
    ...
    where bm.company_id=p_company_id and bm.business_unit_id=v_bu and bm.user_id=auth.uid() and bm.is_active
    limit 1;
  end if;

  if v_role is null then
    select cm.role,cm.permissions into v_role,v_permissions
    from public.company_memberships cm
    where cm.company_id=p_company_id and cm.user_id=auth.uid() and cm.is_active
    limit 1;
  end if;
  ...
  if v_action in ('view','print','export') then
    return case v_role
      ...
      when 'transport' then p_module in ('transport','accounting','reports','master','settings')
