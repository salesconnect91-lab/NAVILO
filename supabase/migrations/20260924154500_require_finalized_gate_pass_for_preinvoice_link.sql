-- Final commercial links may only target posted invoices or finalized gate passes.
...
begin
 ...
 if p_target_type='sales_invoice' then
  ...
  if v_target_status<>'posted' then raise exception 'Sales invoice must be posted before linking.';end if;
  ...
  if v_target_status<>'posted' then raise exception 'Purchase invoice must be posted before linking.';end if;
  ...
  if v_target_status<>'finalized' then raise exception 'Gate pass must be finalized before linking.';end if;
...
revoke all on function public.link_preinvoice_posting_document(uuid,text,uuid) from public,anon;
grant execute on function public.link_preinvoice_posting_document(uuid,text,uuid) to authenticated;