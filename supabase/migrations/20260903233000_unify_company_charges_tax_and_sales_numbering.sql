...
-- Restore the active Charge Master foundation for clean installs. Existing live
-- databases keep their current table because this creator is idempotent.
create table if not exists public.charge_master (
  id uuid primary key default gen_random_uuid(),
  company_id uuid references public.companies(id) on delete cascade,
  charge_key text not null,
  charge_name text not null,
  charge_name_urdu text,
  charge_type text not null default 'both',
...
alter table public.charge_master
  ...
  add column if not exists charge_name_urdu text,
...
drop policy if exists "charge_master_select_company" on public.charge_master;
create policy "charge_master_select_company"
  on public.charge_master for select to authenticated
  using (company_id = public.current_company_id());

drop policy if exists "charge_master_insert_company" on public.charge_master;
create policy "charge_master_insert_company"
  on public.charge_master for insert to authenticated
  with check (company_id = public.current_company_id());

drop policy if exists "charge_master_update_company" on public.charge_master;
create policy "charge_master_update_company"
  on public.charge_master for update to authenticated
  using (company_id = public.current_company_id())
  with check (company_id = public.current_company_id());

drop policy if exists "charge_master_delete_company" on public.charge_master;
create policy "charge_master_delete_company"
