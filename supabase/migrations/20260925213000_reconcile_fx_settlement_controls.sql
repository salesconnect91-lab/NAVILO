begin;
-- Reconcile settlement controls after FX engine: payment-method/account compatibility and base-currency advances.
...
declare
  ...
  v_entry_no text; v_next bigint; v_payment_text text; v_payment_detail text; v_ap_text text; v_fx_text text;
...
begin
  ...
  select code||' - '||name,detail_type into v_payment_text,v_payment_detail from public.chart_of_accounts
  where id=p_payment_account_id and user_id=v_uid and company_id=v_company and is_active and not is_group and allow_manual_entries and detail_type in ('Cash on Hand','Bank Account');
...
declare
  ...
  v_entry_no text; v_next bigint; v_payment_text text; v_payment_detail text; v_ar_text text; v_fx_text text;
  v_method text:=lower(coalesce(nullif(btrim(p_payment_method),''),'cash'));
...
begin
  ...
  select code||' - '||name,detail_type into v_payment_text,v_payment_detail from public.chart_of_accounts where id=p_payment_account_id and user_id=v_uid and company_id=v_company and is_active and not is_group and allow_manual_entries and detail_type in ('Cash on Hand','Bank Account');
  if v_payment_text is null then raise exception 'Payment account must be an active posting Cash on Hand or Bank Account.'; end if;
  if v_method='cash' and v_payment_detail<>'Cash on Hand' then raise exception 'Cash payment method requires a Cash on Hand account.'; end if;
  if v_method in ('bank','cheque','online') and v_payment_detail<>'Bank Account' then raise exception '% payment method requires a Bank Account.',initcap(v_method); end if;
  ...
  v_cash_base:=round(v_amount*v_settlement_rate,2);
  if v_currency=v_base and v_amount>v_alloc then v_ar_base:=v_ar_base+round(v_amount-v_alloc,2); end if;
  v_fx:=round(v_cash_base-v_ar_base,2);
  ...
  if v_currency=v_base and v_amount>v_alloc then
    insert into public.journal_lines(user_id,company_id,business_unit_id,operating_location_id,entry_id,account_id,account,debit,credit,party_type,party_id,party_name,source_debit,source_credit,amount_basis,source_exchange_rate)
    values(v_uid,v_company,v_bu,v_loc,v_journal,v_ar,v_ar_text,0,round(v_amount-v_alloc,2),'customer',v_customer.id,v_customer.name,0,round(v_amount-v_alloc,2),'source_currency',1);
  end if;
  ...
  return jsonb_build_object('success',true,'entry_no',v_entry_no,'journal_entry_id',v_journal,'payment_amount',v_amount,
    'allocated_amount',v_alloc,'advance_amount',round(v_amount-v_alloc,2),'currency_code',v_currency,'settlement_rate',v_settlement_rate,
