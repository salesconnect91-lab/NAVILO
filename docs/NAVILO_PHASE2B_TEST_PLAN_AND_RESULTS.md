...
### First runner attempt and exact repair

At SHA `885e2cee36cdb838ed7ac6c07ec5da83a6577149`, local endpoint discovery and six Auth Admin user creations succeeded. The first `user_profiles` insert returned HTTP 400, so no company/BU/branch fixture or authorization assertion ran. Migration trace identified a required legacy-compatible column: `20260828150000_0005_professional_erp_foundation.sql` created `user_profiles.user_id uuid not null`, while the later table definition was guarded by `if not exists`. The repaired runner writes each synthetic Auth UUID to both `id` and `user_id`. It also replaces the Windows shell invocation with direct `npx.cmd` and sanitizes any future error message. Rerun evidence is required; no negative test has yet passed or failed.

The following run at `ee1f8623aa458bdb980b7cbf02d96f7e91fcb317` exposed that direct `.cmd` execution with `spawnSync(..., shell:false)` is not portable on the user's Windows/Node 24 environment. The runner now uses `ComSpec` for one fixed, non-interpolated status command while retaining `shell:false`; it does not expose stderr or credentials. This attempt ended before provisioning or assertions and changes no security result.
