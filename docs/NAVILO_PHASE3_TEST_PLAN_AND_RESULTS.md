...
## Fifth Windows execution — consolidated descriptions and BU costing

At exact SHA `2499db182d168f224af9037114ea54f8d4452f43`, local migrations were up to date and the matrix again returned **16/19 PASS**. The earlier main-line description and snapshot-trigger failures were absent. Purchase reached `apply_inventory_cost_in()` and failed with SQLSTATE `42P10`; sales reached its consolidated-line lookup and failed with `42703: column hl.description does not exist`. Returns stayed prerequisite-blocked.
