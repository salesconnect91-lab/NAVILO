...
At the reviewed head, migration version/sanity/dependency/object-order checks found **0** issues and **29/29** checker tests passed. TypeScript passed. Application tests initially failed **80/81** because the language runtime test mock lacked the new authenticated `user_language_preferences.eq(...)` query; actual loading fell back to English inside the test. The mock now implements that query with no personal override; **19 test files/81 tests PASS** and the Vite build passes. The primary JS chunk remains **3,147.04 kB (874.38 kB gzip)**, above Vite's 500 kB warning threshold. Dependencies were reused from an identical lockfile because offline `npm ci` lacked `yallist-3.1.1` in the cache.

After Phase 3 evidence, the branch added UI shell/dashboard, language and print changes across many screens. A dark theme and density option were subsequently disabled/removed in commits; complete theme/density is **not verified**. No authenticated multi-viewport visual walkthrough or print/PDF/export acceptance was executed in this checkpoint. Current branch fresh database replay, isolated backup/restore, exhaustive 103-function dynamic security coverage, AMK UAT and production release gates remain **OPEN**. Docker is unavailable in this executor and the user's PC is unavailable; no hosted database test was substituted.
...
The second execution at `5b93352…` proved that foundation and customer AR linking, then stopped before assertion 1 because the test fixture sent `user_id` to `warehouses`. Repository/live catalog show warehouses and godowns intentionally have no such column. The runner payload is corrected to use company scope and authenticated defaults; no database change is warranted. Business UAT is still pending, with zero assertions executed so far.

The third execution at `0e119a5…` completed fixture setup but all five test areas failed before domain assertions: main sales/purchase inserts exposed a shared trigger's invalid `NEW.invoice_no` reference, manual journal exposed invalid `NEW.source_id`, and returns depended on the blocked postings. Read-only catalog comparison confirms sales/purchase use `order_no`, journal uses `source_document_id`, and stock uses `source_id`. Development migration `20260923183710_harden_polymorphic_trigger_row_contracts.sql` restores row-shape-safe branching for the three shared trigger functions without inventing columns or changing production. Static/type/test/build gates pass; the migration and Phase 3 matrix still require a Windows local rerun.

The fourth execution at `01e0dd1…` proves that repair applied and raised Phase 3 to **16/19 PASS**. All executed security, role, journal, reversal, immutability and period-control checks passed. Remaining failures are two root causes plus dependent returns: missing line `description` foundation blocks sales posting, and AFTER-status name snapshotting conflicts with the posted purchase-line lock. Development migration `20260923185524_restore_line_descriptions_and_prelock_snapshots.sql` restores both live-evidenced description columns and moves sales/purchase line snapshots before the status transition without weakening posted locks. Full sales/purchase/stock/AR/AP/return verification remains pending local rerun.
...
## Phase 3 fifth Windows execution — remaining posting/catalog drift

At exact SHA `2499db182d168f224af9037114ea54f8d4452f43`, the previous main-line description and snapshot errors disappeared, but the matrix remained **16/19 PASS**. Purchase reached inventory costing and failed on SQLSTATE `42P10`: repository migration `20260905013001` changed uniqueness to `(company_id,business_unit_id,item_id)`, while replay retained `apply_inventory_cost_in()` with `ON CONFLICT (user_id,item_id)`. Sales reached the consolidated/Hawala lookup but failed on absent `consolidated_sales_invoice_lines.description`. Returns remained prerequisite-blocked rather than independently failed.

Read-only production history/catalog confirms nullable `description text` on both consolidated sales and purchase lines and company/BU-aware inventory-cost functions. Development migration `20260923190959_restore_consolidated_descriptions_and_bu_inventory_cost.sql` restores those exact columns and evidenced functions. The functions retain pinned `search_path`, module permission checks and service-role-only direct execution. Static migration gates, 28 checker tests, TypeScript, 19 files/81 application tests and production build pass; the known 3,150.13 kB main-bundle warning remains. Windows local execution and the 19-case matrix rerun remain pending, so sales/purchase/returns are not yet verified complete.

## Phase 3 sixth Windows execution — purchase path verified, payment/sales gaps isolated

At exact development SHA `9f295b38f26c150e3fc10afe052d870cdfa4c310`, local migration history was up to date and the expanded matrix measured **27/32 PASS**. The new purchase foundation is verified: valid purchase posting, totals/outstanding, line snapshots, stock increase, balanced AP journal, duplicate-post atomicity and posted header edit/delete denial all passed. Previously verified tenant/role denials, manual journal lifecycle and period controls remained green.

Five failed rows collapse to two independent defects and three prerequisites. Supplier payment creates a company-owned journal but the restored `post_journal_entry` lookup also demands `legacy_data_user_id()`; an accounts-role caller therefore receives `Journal entry not found` before allocation/status update. Its reversal call omits the unavailable journal ID and is consequently a dependent PostgREST signature failure. Sales now advances past consolidated descriptions but clean replay lacks the live-evidenced `sales_order_charges.cost_account_id` (and related cost fields); returns remain dependent on sales posting.

Development migration `20260923193415_restore_sales_charge_cost_and_journal_tenant_scope.sql` restores the exact charge-cost columns/constraint and changes the journal lock boundary to current company + BU + branch while deriving the stored owner from the locked row. Accounting/post permission and downstream owner-scoped validations remain intact. The runner also stops accepting an arbitrary denial as proof of overpayment or duplicate-reversal correctness; it now requires the expected business error.

Post-change non-database gates pass: zero migration version/sanity/dependency/order findings, 29 checker tests, TypeScript, 19 files/81 application tests and the Vite build. The existing 3,150.13 kB primary bundle warning remains. Actual local migration and behavior verification are still pending.
