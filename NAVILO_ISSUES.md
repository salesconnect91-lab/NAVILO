...
| MIG-18 / P0 | **CONFIRMED BUG; development patch prepared.** Third run at `0e119a5…` reached all five areas but main sales/purchase inserts failed on nonexistent `NEW.invoice_no`, manual journal failed on nonexistent `NEW.source_id`, and returns were prerequisite-blocked. The shared trigger functions mixed table-specific row fields in one SQL expression/compound condition. Live read-only catalog confirms the column contracts and two safer production definitions; production's transaction-link synchronizer retains a latent equivalent risk. | Apply `20260923183710_harden_polymorphic_trigger_row_contracts.sql` locally only. It uses JSON document-number extraction and procedural table branches, retains pinned `search_path` and revoked direct execution. Acceptance: migration applies, purchase/sales/manual-journal inserts pass the trigger boundary, then the complete Phase 3 expected/actual matrix runs. |
| TEST-11 / P0 | **EXECUTED, FAILING/BLOCKED.** Third run reported 0/5 areas passed, but the evidence collapses to MIG-18 plus dependent returns rather than five independent business defects. No stock/GL/AR/AP/immutability/period assertion is yet verified. | Rerun after MIG-18; record individual assertions and fix only newly confirmed domain failures. |
