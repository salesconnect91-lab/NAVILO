...
export default function ConsolidatedInvoices() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        setError(e?.message || "Failed to load Consolidated Sales Invoice invoices.");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const saveDraft = async () => {
    if (isLocked) return setError("Posted Consolidated Sales Invoice document is immutable.");
    ...
    try {
      ...
      if (!invoiceId) throw new Error("Unable to create Consolidated Sales Invoice document.");
      ...
      setError(e?.message || "Failed to save Consolidated Sales Invoice document.");
      return null;
    } finally {
      setSaving(false);
    }
  ...
  const deleteDraft = async () => {
    if (!editingId || isLocked || !window.confirm("Delete this Consolidated Sales Invoice draft?")) return;
    setSaving(true);
    try {
  ...
  if (showForm) {
    return <div className="navilo-consolidated-sales-editor space-y-4" data-navilo-document-editor="true" data-navilo-commercial-standard="true">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-3"><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4"><div><label className="label">Invoice Type</label><SearchableSelect className="input" disabled={isLocked} value={invoiceType} onChange={(e) => changeInvoiceType(e.target.value as InvoiceType)}><option value="Sale Invoice">Without Tax</option><option value="Tax Invoice">With Tax</option></SearchableSelect></div><div><label className="label">Invoice No.</label><input className="input bg-slate-50 font-semibold" readOnly value={invoiceNo} /></div><div><label className="label">Invoice Date</label><input className="input" type="date" disabled={isLocked} value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} /></div><div><label className="label">Fixed VAT %</label><input className="input bg-slate-50 text-right" readOnly value={invoiceType === "Tax Invoice" ? configuredTaxRate || "Not configured" : "0"} /></div><div><label className="label">Customer</label><SearchableSelect className="input" disabled={isLocked} value={customerId} onChange={(e) => setCustomerId(e.target.value)}><option value="">— Select customer —</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</SearchableSelect></div><div><label className="label">Hawala / Reference Name</label><input className="input" disabled={isLocked} value={referenceName} onChange={(e) => setReferenceName(e.target.value)} /></div><div><label className="label">Reference No.</label><input className="input" disabled={isLocked} value={referenceNo} onChange={(e) => setReferenceNo(e.target.value)} /></div><div><label className="label">Remarks</label><input className="input" disabled={isLocked} value={referenceNotes} onChange={(e) => setReferenceNotes(e.target.value)} /></div></div></section>
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white"><div className="flex items-center justify-between border-b border-slate-200 px-3 py-2.5"><div><div className="text-[12px] font-semibold text-slate-800">Invoice Items</div><div className="text-[12px] text-slate-400">Item, description, godown, quantity, rate and fixed VAT</div></div>{!isLocked && <button type="button" className="btn-secondary" onClick={() => setRows((current) => [...current, emptyRow(globalTaxPercent, godowns[0]?.id || "")])}><Plus className="h-3.5 w-3.5" />Add Row</button>}</div><div className="space-y-3 p-3 md:hidden">{rows.map((row,index)=>{const base=(Number(row.qty)||0)*(Number(row.rate)||0);const tax=invoiceType==="Tax Invoice"?base*(Number(row.tax_percent)||0)/100:0;return <div key={`mobile-${index}`} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><div className="mb-3 flex items-center justify-between"><strong className="text-sm">Item {index+1}</strong>{!isLocked&&rows.length>1&&<button type="button" className="text-rose-600" onClick={()=>setRows(current=>current.filter((_,rowIndex)=>rowIndex!==index))}><Trash2 className="h-4 w-4"/></button>}</div><div className="space-y-3"><label className="block text-xs font-semibold text-slate-600">Item<SearchableSelect className="input mt-1" disabled={isLocked} value={row.item_id} onChange={(e)=>updateRow(index,"item_id",e.target.value)}><option value="">— Select item —</option>{items.map(item=><option key={item.id} value={item.id} data-search={item.sku||undefined}>{item.name}</option>)}</SearchableSelect></label><label className="block text-xs font-semibold text-slate-600">Description<input className="input mt-1" disabled={isLocked} value={row.description} onChange={(e)=>updateRow(index,"description",e.target.value)}/></label><label className="block text-xs font-semibold text-slate-600">Godown<SearchableSelect className="input mt-1" disabled={isLocked} value={row.godown_id} onChange={(e)=>updateRow(index,"godown_id",e.target.value)}><option value="">— Select godown —</option>{godowns.map(godown=><option key={godown.id} value={godown.id}>{godown.name}</option>)}</SearchableSelect></label><div className="grid grid-cols-2 gap-3"><label className="block text-xs font-semibold text-slate-600">Qty<input className="input mt-1 text-right" type="number" step="0.001" disabled={isLocked} value={row.qty} onChange={(e)=>updateRow(index,"qty",e.target.value)}/></label><label className="block text-xs font-semibold text-slate-600">Rate<input className="input mt-1 text-right" type="number" step="0.01" disabled={isLocked} value={row.rate} onChange={(e)=>updateRow(index,"rate",e.target.value)}/></label></div>{invoiceType==="Tax Invoice"&&<div className="flex justify-between text-sm"><span>VAT {Number(row.tax_percent)||0}%</span><span>{money(tax)}</span></div>}<div className="flex justify-between border-t pt-3 text-sm"><span className="text-slate-500">Amount</span><strong>{money(base+tax)}</strong></div></div></div>;})}</div><div className="hidden overflow-x-auto md:block"><div data-report-content data-navilo-customizable="true" className="contents"><table className="w-full min-w-[940px] text-[12px]"><thead className="bg-slate-50"><tr className="border-b border-slate-200"><th className="px-3 py-2 text-left">Item</th><th className="px-2 py-2 text-left">Description</th><th className="px-2 py-2 text-left">Godown</th><th className="px-2 py-2 text-right">Qty</th><th className="px-2 py-2 text-right">Rate</th>{invoiceType === "Tax Invoice" && <th className="px-2 py-2 text-right">VAT %</th>}<th className="px-2 py-2 text-right">Amount</th><th className="w-10" /></tr></thead><tbody>{rows.map((row,index) => { const base=(Number(row.qty)||0)*(Number(row.rate)||0); const tax=invoiceType === "Tax Invoice" ? base*(Number(row.tax_percent)||0)/100 : 0; return <tr key={index} className="border-b border-slate-100"><td className="px-3 py-2"><SearchableSelect className="input" disabled={isLocked} value={row.item_id} onChange={(e) => updateRow(index,"item_id",e.target.value)}><option value="">— Select item —</option>{items.map((item) => <option key={item.id} value={item.id} data-search={item.sku||undefined}>{item.name}</option>)}</SearchableSelect></td><td className="px-2 py-2"><input className="input" disabled={isLocked} value={row.description} onChange={(e) => updateRow(index,"description",e.target.value)} /></td><td className="px-2 py-2"><SearchableSelect className="input" disabled={isLocked} value={row.godown_id} onChange={(e) => updateRow(index,"godown_id",e.target.value)}><option value="">— Select godown —</option>{godowns.map((godown) => <option key={godown.id} value={godown.id}>{godown.name}</option>)}</SearchableSelect></td><td className="px-2 py-2"><input className="input text-right" type="number" step="0.001" disabled={isLocked} value={row.qty} onChange={(e) => updateRow(index,"qty",e.target.value)} /></td><td className="px-2 py-2"><input className="input text-right" type="number" step="0.01" disabled={isLocked} value={row.rate} onChange={(e) => updateRow(index,"rate",e.target.value)} /></td>{invoiceType === "Tax Invoice" && <td className="px-2 py-2"><input className="input bg-slate-50 text-right" readOnly value={row.tax_percent} /></td>}<td className="px-2 py-2 text-right font-semibold">{money(base+tax)}</td><td className="px-2 py-2">{!isLocked && rows.length>1 && <button type="button" className="text-rose-600" onClick={() => setRows((current) => current.filter((_,rowIndex)=>rowIndex!==index))}><Trash2 className="h-3.5 w-3.5" /></button>}</td></tr>; })}</tbody></table></div></div></section>
