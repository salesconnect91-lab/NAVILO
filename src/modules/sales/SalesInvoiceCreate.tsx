...
export default function SalesInvoiceCreate() {
  ...
  return (
    <div className="navilo-invoice-editor space-y-4" data-navilo-commercial-standard="true" data-navilo-document-editor="true">
      ...
      <section className="navilo-invoice-meta rounded-lg border border-slate-200 bg-white p-4">
      ...
      <section className="navilo-invoice-lines overflow-hidden rounded-lg border border-slate-200 bg-white">
        ...
        <div className="divide-y divide-slate-100 md:hidden">
          {rows.map((row,index) => {
            const base=(Number(row.qty)||0)*(Number(row.rate)||0);
            const lineTax=invoiceType === "Tax Invoice" ? base*(Number(row.tax_percent)||0)/100 : 0;
            return <div key={index} className="space-y-3 p-3">
              <div className="flex items-center justify-between"><span className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Item {index + 1}</span>{!isLocked && rows.length>1 && <button type="button" className="inline-flex items-center gap-1 text-[12px] font-semibold text-rose-600" onClick={() => setRows((current) => current.filter((_,rowIndex) => rowIndex !== index))}><Trash2 className="h-3.5 w-3.5" />Remove</button>}</div>
              <div><label className="label">Item</label><SearchableSelect className="input" disabled={isLocked} value={row.item_id} onChange={(e) => updateRow(index,"item_id",e.target.value)}><option value="">— Select item —</option>{items.map((item) => <option key={item.id} value={item.id} data-search={item.sku||undefined}>{item.name}</option>)}</SearchableSelect></div>
              <div><label className="label">Description</label><input className="input" disabled={isLocked} value={row.description} onChange={(e) => updateRow(index,"description",e.target.value)} /></div>
              <div><label className="label">Godown</label><SearchableSelect className="input" disabled={isLocked} value={row.godown_id} onChange={(e) => updateRow(index,"godown_id",e.target.value)}><option value="">— Select godown —</option>{godowns.map((godown) => <option key={godown.id} value={godown.id}>{godown.name}</option>)}</SearchableSelect></div>
              <div className="grid grid-cols-2 gap-2"><div><label className="label">Qty</label><input className="input text-right" type="number" step="0.001" disabled={isLocked} value={row.qty} onChange={(e) => updateRow(index,"qty",e.target.value)} /></div><div><label className="label">Rate</label><input className="input text-right" type="number" step="0.01" disabled={isLocked} value={row.rate} onChange={(e) => updateRow(index,"rate",e.target.value)} /></div></div>
              <div className="flex items-end justify-between rounded-lg bg-slate-50 px-3 py-2">{invoiceType === "Tax Invoice" && <div><div className="text-[11px] text-slate-400">VAT</div><div className="font-semibold">{row.tax_percent}%</div></div>}<div className="ml-auto text-right"><div className="text-[11px] text-slate-400">Line Total</div><div className="font-bold text-slate-900">{formatCurrency(base+lineTax)}</div></div></div>
            </div>;
          })}
        </div>
        <div className="hidden md:block"><table className="navilo-invoice-lines-table w-full table-fixed text-[12px]"><thead className="bg-slate-50"><tr className="border-b border-slate-200"><th className="px-3 py-2 text-left">Item</th><th className="px-2 py-2 text-left">Description</th><th className="px-2 py-2 text-left">Godown</th><th className="px-2 py-2 text-right">Qty</th><th className="px-2 py-2 text-right">Rate</th>{invoiceType === "Tax Invoice" && <th className="px-2 py-2 text-right">VAT %</th>}<th className="px-2 py-2 text-right">Amount</th><th className="w-10" /></tr></thead><tbody>{rows.map((row,index) => { const base=(Number(row.qty)||0)*(Number(row.rate)||0); const lineTax=invoiceType === "Tax Invoice" ? base*(Number(row.tax_percent)||0)/100 : 0; return <tr key={index} className="border-b border-slate-100"><td className="px-3 py-2"><SearchableSelect className="input" disabled={isLocked} value={row.item_id} onChange={(e) => updateRow(index,"item_id",e.target.value)}><option value="">— Select item —</option>{items.map((item) => <option key={item.id} value={item.id} data-search={item.sku||undefined}>{item.name}</option>)}</SearchableSelect></td><td className="px-2 py-2"><input className="input" disabled={isLocked} value={row.description} onChange={(e) => updateRow(index,"description",e.target.value)} /></td><td className="px-2 py-2"><SearchableSelect className="input" disabled={isLocked} value={row.godown_id} onChange={(e) => updateRow(index,"godown_id",e.target.value)}><option value="">— Select godown —</option>{godowns.map((godown) => <option key={godown.id} value={godown.id}>{godown.name}</option>)}</SearchableSelect></td><td className="px-2 py-2"><input className="input text-right" type="number" step="0.001" disabled={isLocked} value={row.qty} onChange={(e) => updateRow(index,"qty",e.target.value)} /></td><td className="px-2 py-2"><input className="input text-right" type="number" step="0.01" disabled={isLocked} value={row.rate} onChange={(e) => updateRow(index,"rate",e.target.value)} /></td>{invoiceType === "Tax Invoice" && <td className="px-2 py-2"><input className="input bg-slate-50 text-right" readOnly value={row.tax_percent} /></td>}<td className="px-2 py-2 text-right font-semibold">{formatCurrency(base+lineTax)}</td><td className="px-2 py-2">{!isLocked && rows.length>1 && <button type="button" className="text-rose-600" onClick={() => setRows((current) => current.filter((_,rowIndex) => rowIndex !== index))}><Trash2 className="h-3.5 w-3.5" /></button>}</td></tr>; })}</tbody></table></div>
      </section>

      <section className="navilo-invoice-charges rounded-lg border border-slate-200 bg-white">
      ...
      <Modal open={previewOpen} title={`${invoiceType === "Tax Invoice" ? "With Tax" : "Without Tax"} Sales Invoice Preview — ${invoiceNo}`} onClose={() => setPreviewOpen(false)}>
        <div className="p-2">
          <PrintLayout
            ...
            extraFields={[{ label: "Sales Person", value: salesPerson || "—" }, { label: "Reference", value: [referenceName, referenceNo].filter(Boolean).join(" · ") || "—" }, ...(discountValue > 0 ? [{ label: "Discount", value: formatCurrency(discountValue) }] : [])]}
