...
export default function ChargeMaster() {
  ...
  const [showUrdu, setShowUrdu] = useState(() => { const root=document.documentElement; return root.dataset.primaryLanguage==="ur" || (root.dataset.languageMode==="bilingual" && root.dataset.secondaryLanguage==="ur"); });
  ...
  return <div className="space-y-5" data-navilo-master-standard="true">
    ...
    <div data-report-content data-navilo-print-surface>
      <DataTable<Charge> loading={loading} rows={visibleCharges} emptyMessage="No charges found." columns={[
        {key:"charge_name",label:"Charge",render:c=><div><div className="font-medium">{c.charge_name}</div><div className="text-xs text-slate-400">{c.charge_key} · {c.charge_type}</div></div>},
        ...(showUrdu?[{key:"charge_name_urdu",label:"Urdu Name",render:(c:Charge)=><span dir="rtl">{c.charge_name_urdu||"—"}</span>}]:[]),
        {key:"default_rate",label:"Basis / Rate",render:c=><div><div>{unitLabel(c.unit)}</div><div className="text-xs text-slate-500">{c.unit==="manual"?"Entered on document":`Rate ${Number(c.default_rate||0).toLocaleString()}${c.is_fixed?" · Locked":""}`}</div></div>},
        {key:"applies_to",label:"Applies To",render:c=><span className="capitalize">{c.applies_to}</span>},
        {key:"purchase_treatment",label:"Purchase Treatment",render:c=>c.applies_to==="sales"?"—":c.purchase_treatment==="expense"?"Expense":"Landed Cost / Inventory"},
        {key:"revenue_account_id",label:"Revenue COA",render:c=>accountLabel(c.revenue_account_id)},
        {key:"cost_account_id",label:"Cost COA",render:c=>accountLabel(c.cost_account_id)},
        {key:"tax_applicable",label:"Tax",render:c=>c.tax_applicable?"Yes":"No"},
        {key:"is_active",label:"Status",render:c=>c.is_active?"Active":"Inactive"},
        {key:"actions",label:"Actions",sortable:false,render:c=><div className="flex justify-end gap-2">{canEdit&&<button onClick={()=>startEdit(c)} className="rounded border p-2" aria-label="Edit charge"><Pencil size={15}/></button>}{canEdit&&<button onClick={()=>void toggle(c)} className="rounded border p-2" aria-label={c.is_active?"Deactivate charge":"Activate charge"}><Power size={15}/></button>}{canDelete&&<button onClick={()=>void remove(c)} className="rounded border p-2 text-red-600" aria-label="Delete charge"><Trash2 size={15}/></button>}</div>}
    ...
    <Modal open={open} title={editingId ? "Edit Charge" : "Add Charge"} onClose={() => !saving && setOpen(false)}>
      <form onSubmit={save} className="space-y-4">
        ...
        {showUrdu&&<div data-language-code="ur"><div className="mb-1 flex items-center justify-between"><label className="label mb-0">Urdu Name</label><button type="button" className="btn-secondary text-xs" onClick={() => setForm((current) => ({ ...current, charge_name_urdu: toUrduName(current.charge_name) }))}>Auto Urdu</button></div><input dir="rtl" className="input text-right" value={form.charge_name_urdu} onChange={(e) => setForm((current) => ({ ...current, charge_name_urdu: e.target.value }))} /></div>}
