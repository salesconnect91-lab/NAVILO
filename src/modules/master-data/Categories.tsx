...
export default function Categories(){
 ...
 return <div className="space-y-4" data-navilo-master-standard="true">
   ...
   <div className="space-y-3" data-no-print data-no-export>
     ...
     {activeFilterCount>0&&<div className="flex flex-wrap gap-2" data-report-filter-value data-report-filter-label="Active Filters">{showUrdu&&translationFilter!=="all"&&<button type="button" className="rounded-full border bg-white px-3 py-1 text-xs font-medium" onClick={()=>setTranslationFilter("all")}>Urdu: {translationFilter} ×</button>}{descriptionFilter!=="all"&&<button type="button" className="rounded-full border bg-white px-3 py-1 text-xs font-medium" onClick={()=>setDescriptionFilter("all")}>Description: {descriptionFilter} ×</button>}<button type="button" className="text-xs font-semibold text-primary-600" onClick={clearFilters}>Clear all</button></div>}
