...
export default function Employees() {
  ...
  return <div className="space-y-4" data-navilo-master-standard="true">
    <div className="rounded-xl border bg-white p-4 shadow-sm flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-xl font-bold">Employees</h1><p className="text-xs text-slate-500">Manage employee master records</p></div>{canCreate&&<button className="btn-primary" onClick={openAdd}><Plus className="h-4 w-4" /> Add Employee</button>}</div>
    ...
    <div data-report-content data-navilo-print-surface className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center gap-2" data-no-print data-no-export><div className="relative flex-1 min-w-48"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input className="input w-full pl-9" placeholder="Search employees" value={search} onChange={(event) => setSearch(event.target.value)} /></div><select className="input" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All Status</option><option value="active">Active</option><option value="inactive">Inactive</option></select>{(search||status!=="all")&&<button className="btn" onClick={()=>{setSearch("");setStatus("all")}}>Clear Filters</button>}<button className="btn" onClick={() => void load()}>Refresh</button></div>
    {status!=="all"&&<div className="mt-3 flex flex-wrap gap-2" data-report-filter-value data-report-filter-label="Active Filters"><button type="button" className="rounded-full border bg-white px-3 py-1 text-xs font-medium" onClick={()=>setStatus("all")}>Status: {status} ×</button></div>}
    <div className="mt-4"><DataTable<Employee> loading={loading} rows={filtered} emptyMessage="No employees found." columns={[
      {key:"employee_code",label:"Code",render:e=>e.employee_code||"—"},
      {key:"name",label:"Employee"},
      ...(showUrdu?[{key:"name_urdu",label:"Urdu Name",render:(e:Employee)=><span data-language-code="ur" dir="rtl">{e.name_urdu||"—"}</span>}]:[]),
      ...
      {key:"is_active",label:"Status",render:e=>canEdit?<button type="button" className="btn" onClick={()=>void toggle(e)}>{e.is_active?"Active":"Inactive"}</button>:<span>{e.is_active?"Active":"Inactive"}</span>},
      {key:"actions",label:"Actions",sortable:false,render:e=>canEdit?<button className="btn" title="Edit" onClick={()=>openEdit(e)}><Pencil className="h-4 w-4" /></button>:<span className="text-slate-400">—</span>}
    ] satisfies Column<Employee>[]} /></div><p className="mt-3 text-xs text-slate-500">Showing {filtered.length} of {employees.length} employees</p></div>
