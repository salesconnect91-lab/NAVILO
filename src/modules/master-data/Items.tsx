...
type ColumnKey="sku"|"item"|"urdu"|"type"|"category"|"warehouse"|"hs"|"unit"|"size"|"grade"|"cost"|"price";

const EMPTY={sku:"",name:"",name_urdu:"",type:"finished" as ItemType,grade:"",size:"",unit:"",hs_code:"",cost:"0",price:"0",category_id:"",warehouse_id:""};
const DEFAULT_COLUMNS:Record<ColumnKey,boolean>={sku:true,item:true,urdu:true,type:true,category:true,warehouse:false,hs:true,unit:true,size:true,grade:true,cost:true,price:true};
const COLUMN_LABELS:Record<ColumnKey,string>={sku:"SKU",item:"Item",urdu:"Urdu Name",type:"Type",category:"Category",warehouse:"Warehouse",hs:"HS/PCT",unit:"UOM",size:"Size",grade:"Grade",cost:"Cost",price:"Sale Price"};
...
export default function Items(){
  const[items,setItems]=useState<Item[]>([]),[categories,setCategories]=useState<Category[]>([]),[warehouses,setWarehouses]=useState<Warehouse[]>([]),[uoms,setUoms]=useState<Uom[]>([]);
  const[form,setForm]=useState(EMPTY),[edit,setEdit]=useState<Item|null>(null),[open,setOpen]=useState(false),[search,setSearch]=useState("");
  ...
  return <div className="space-y-4">
    ...
    <section data-report-content className="space-y-3 rounded-xl bg-white print:shadow-none">
      <div className="border-b border-slate-200 px-4 py-4">
        ...
        <div className="mt-1 text-xs text-slate-500">{shown.length} item(s) • {typeFilter==="all"?"All types":typeFilter} • {categoryFilter==="all"?"All categories":cat(categoryFilter)?.name||"Selected category"} • {warehouseFilter==="all"?"All warehouses":wh(warehouseFilter)?.name||"Selected warehouse"}</div>
      ...
      </tr>)}</tbody><tfoot><tr className="border-t bg-slate-50 font-semibold"><td className="p-3" colSpan={Math.max(1,Object.values(columns).filter(Boolean).length-2)}>TOTAL / SUMMARY</td>{visible("cost")&&<td className="p-3 text-right">{totalCost.toLocaleString()}</td>}{visible("price")&&<td className="p-3 text-right">{totalPrice.toLocaleString()}</td>}<td data-no-print data-no-export/></tr></tfoot></table></div>
    </section>
    ...
    </div><div className="flex justify-end gap-2 pt-2"><button type="button" className="btn-secondary" onClick={()=>{setOpen(false);setEdit(null)}}>Cancel</button><button type="submit" className="btn-primary" disabled={saving}>{saving?"Saving...":"Save Item"}</button></div></form></div>}
  </div>;
}
