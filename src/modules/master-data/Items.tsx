...
type ItemForm = {
  ...
  cost:string; price:string; category_id:string; warehouse_id:string;
};

const EMPTY:ItemForm={sku:"",name:"",name_urdu:"",type:"finished",grade:"",size:"",unit:"",hs_code:"",cost:"0",price:"0",category_id:"",warehouse_id:""};
...
export default function Items(){
  ...
  const shown=useMemo(()=>items.filter(x=>(typeFilter==="all"||x.type===typeFilter)&&(categoryFilter==="all"||x.category_id===categoryFilter)&&(warehouseFilter==="all"||x.warehouse_id===warehouseFilter)&&(!search||[x.sku,x.name,showUrdu?x.name_urdu:null,x.grade,x.size,x.hs_code,x.unit,cat(x.category_id)?.name,showUrdu?cat(x.category_id)?.name_urdu:null,wh(x.warehouse_id)?.name,showUrdu?wh(x.warehouse_id)?.name_urdu:null].some(v=>n(v).includes(n(search))))),[items,search,typeFilter,categoryFilter,warehouseFilter,categories,warehouses,showUrdu]);
  ...
  const start=async()=>{try{setEdit(null);setNameManual(false);const defaultUnit=uoms.find(u=>n(u.symbol)==="kg")?.symbol??uoms[0]?.symbol??"";setForm({...EMPTY,sku:await nextSku("finished"),warehouse_id:warehouses[0]?.id??"",unit:defaultUnit});setOpen(true)}catch(x){setError(x instanceof Error?x.message:"SKU generation failed")}};
  const save=async(e:FormEvent)=>{
    ...
    try{
      ...
      const p={sku,name:resolvedName,name_urdu:form.name_urdu.trim()||toUrduName(resolvedName),type:form.type,grade:form.grade.trim()||null,size:form.size.trim()||null,unit:form.unit.trim()||null,hs_code:form.hs_code.trim()||null,cost:num(form.cost),price:num(form.price),category_id:form.category_id||null,warehouse_id:form.warehouse_id||null};
