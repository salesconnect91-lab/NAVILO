...
export default function ConsolidatedPurchaseInvoices() {
  ...
  return <div className="navilo-purchase-neus" data-navilo-commercial-standard="true">
      ...
      {showForm && <form ref={formRef} onSubmit={save} className="navilo-consolidated-purchase-editor mb-6 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {editingId && currentInvoice?.status === "draft" && <button type="button" className="btn-danger" disabled={deleting} onClick={() => void deleteDraft(editingId, invoiceNo)}>{deleting ? "Deleting..." : "Delete Draft"}</button>}
        ...
        <div className="space-y-3 md:hidden">{rows.map((row,index)=>{const base=n(row.qty)*n(row.unit_cost);const tax=invoiceType==="Tax Invoice"?base*n(row.tax_percent)/100:0;return <div key={`${row.id||"new"}-mobile-${index}`} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><div className="mb-3 flex items-center justify-between"><strong className="text-sm">Item {index+1}</strong>{!locked&&<button type="button" className="text-rose-600" onClick={()=>removeLine(index)} aria-label="Remove line">×</button>}</div><div className="space-y-3"><label className="block text-xs font-semibold text-slate-600">Item<SearchableSelect className="input mt-1 w-full" disabled={locked} value={row.item_id} onChange={(e)=>updateRow(index,"item_id",e.target.value)}><option value="">— Select —</option>{items.map((item)=><option key={item.id} value={item.id} data-search={item.sku||undefined}>{item.name}</option>)}</SearchableSelect></label><label className="block text-xs font-semibold text-slate-600">Description<input className="input mt-1 w-full" disabled={locked} value={row.description} placeholder="Optional description" onChange={(e)=>updateRow(index,"description",e.target.value)}/></label><label className="block text-xs font-semibold text-slate-600">Godown<SearchableSelect className="input mt-1 w-full" disabled={locked} value={row.godown_id} onChange={(e)=>updateRow(index,"godown_id",e.target.value)}><option value="">— Select —</option>{godowns.map((godown)=><option key={godown.id} value={godown.id}>{godown.name}{godown.name_urdu?` / ${godown.name_urdu}`:""}</option>)}</SearchableSelect></label><div className="grid grid-cols-2 gap-3"><label className="block text-xs font-semibold text-slate-600">Qty<input className="input mt-1 text-right" type="number" min="0.001" step="0.001" disabled={locked} value={row.qty} onChange={(e)=>updateRow(index,"qty",e.target.value)}/></label><label className="block text-xs font-semibold text-slate-600">Unit Cost<input className="input mt-1 text-right" type="number" min="0" step="0.01" disabled={locked||Boolean(row.order_book_commitment_id)} value={row.unit_cost} onChange={(e)=>updateRow(index,"unit_cost",e.target.value)}/></label></div>{invoiceType==="Tax Invoice"&&<div className="flex justify-between text-sm"><span>VAT {n(row.tax_percent)}%</span><span>{formatCurrency(tax)}</span></div>}<div className="flex justify-between border-t pt-3 text-sm"><span className="text-slate-500">Amount</span><strong>{formatCurrency(base+tax)}</strong></div></div></div>;})}</div>
        <div className="hidden overflow-x-auto rounded-lg border border-slate-200 md:block">
          <div data-report-content data-navilo-customizable="true" className="contents"><table className="w-full min-w-[1220px] table-fixed text-sm">
            ...
            <tbody>{rows.map((row, index) => {
              ...
              return <tr key={`${row.id || "new"}-${index}`} className="border-b border-slate-100 align-top">
                <td className="p-2"><SearchableSelect className="input w-full" disabled={locked} value={row.item_id} onChange={(e) => updateRow(index, "item_id", e.target.value)}><option value="">— Select —</option>{items.map((item) => <option key={item.id} value={item.id} data-search={item.sku||undefined}>{item.name}</option>)}</SearchableSelect>{row.order_book_commitment_id && <div className="mt-1 text-[11px] font-medium text-primary-600">Order Book linked</div>}</td>
    ...
    {showForm && editingId && <div id="consolidated-purchase-print-root" className="hidden print:block" data-print-root><PrintLayout
      ...
      extraFields={[{ label: "Document", value: "Consolidated Purchase" }, { label: "Status", value: currentInvoice?.status?.toUpperCase() || "DRAFT" }, ...(referenceName ? [{ label: "Reference Name", value: referenceName }] : []), ...(referenceNo ? [{ label: "Reference No", value: referenceNo }] : []), ...(invoiceType === "Tax Invoice" ? [{ label: "VAT Rate", value: `${n(taxPercent)}% (Fixed)` }] : [])]}
