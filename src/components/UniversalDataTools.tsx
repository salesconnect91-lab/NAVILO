...
export default function UniversalDataTools(){
  ...
  const reportMode=isReportPath(pathname),standardPath=!genericReport&&(reportMode||pathname==="/sales/consolidated"||pathname==="/purchase/consolidated"),invoiceEditor=isInvoiceEditorPath(pathname),customizable=reportMode||hasCustomizableTable;
  ...
  useEffect(()=>{const main=document.querySelector<HTMLElement>("#navilo-main-content");if(!main||!reportMode||genericReport){setHasCustomizableTable(false);return;}const scan=()=>{const next=Boolean(main.querySelector("[data-navilo-data-table],[data-navilo-customizable='true']"));setHasCustomizableTable(prev=>prev===next?prev:next)};scan();const o=new MutationObserver(scan);o.observe(main,{childList:true,subtree:true});return()=>{o.disconnect();setHasCustomizableTable(false)}},[pathname,reportMode,genericReport]);
  useEffect(()=>{const main=document.querySelector<HTMLElement>("#navilo-main-content");if(!main||!reportMode||genericReport){setHasLocalDocumentOutput(false);return;}const hidden=new Set<HTMLElement>();const scan=()=>{let template=false,upload=false,documentOutput=false;main.querySelectorAll<HTMLElement>("button,a,[role='button']").forEach(el=>{if(el.closest("[data-navilo-global-data-tools]"))return;if(isDocumentOutputAction(el)){documentOutput=true;return;}const label=labelOf(el);if(isTemplateAction(label)){template=true;el.style.setProperty("display","none","important");hidden.add(el);return}if(isUploadAction(label)){upload=true;el.style.setProperty("display","none","important");hidden.add(el);return}if(isExportDuplicate(label)){el.style.setProperty("display","none","important");el.dataset.naviloDuplicateGlobalAction="true";hidden.add(el)}});setHasTemplate(prev=>prev===template?prev:template);setHasUpload(prev=>prev===upload?prev:upload);setHasLocalDocumentOutput(prev=>prev===documentOutput?prev:documentOutput)};scan();const o=new MutationObserver(scan);o.observe(main,{childList:true,subtree:true});return()=>{o.disconnect();hidden.forEach(el=>{el.style.removeProperty("display");delete el.dataset.naviloDuplicateGlobalAction});setHasTemplate(false);setHasUpload(false);setHasLocalDocumentOutput(false)}},[pathname,reportMode,genericReport]);
  useEffect(()=>{
  ...
  const toolbar=reportMode?<div className="navilo-report-toolbar" ref={ref} data-no-print data-no-export data-navilo-global-data-tools>
  ...
  return standardHost?createPortal(<><ConsolidatedInvoiceTools/>{reportMode&&toolbar}</>,standardHost):null;
}
