...
export default function UniversalDataTools(){
  ...
  const exp=(t:"excel"|"csv"|"word",scope:"filtered"|"selected"="filtered")=>{if(!canExport)return;const root=currentExportRoot();if(!root)return;const title=currentPageTitle(),file=cleanTitle(title);const table=root.querySelector<HTMLElement>("[data-navilo-data-table]")??(root.matches("[data-navilo-data-table]")?root:null);const hidden:HTMLElement[]=[];if(scope==="selected"&&table){table.querySelectorAll<HTMLTableRowElement>("tbody tr").forEach(row=>{const checked=row.querySelector<HTMLInputElement>('input[type="checkbox"][aria-label="Select row"]')?.checked;if(!checked){row.style.setProperty("display","none","important");hidden.push(row)}})}try{if(t==="excel")exportDomReportToExcel(file,root,title);if(t==="csv")exportDomReportToCSV(file,root,title);if(t==="word")exportDomReportToWord(file,root,title)}finally{hidden.forEach(row=>row.style.removeProperty("display"));setOpen(false)}};
