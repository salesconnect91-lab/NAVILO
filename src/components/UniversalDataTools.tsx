...
export default function UniversalDataTools(){
  ...
  useEffect(()=>{const main=document.querySelector<HTMLElement>("#navilo-main-content");if(!main||(!reportMode&&!masterMode)||genericReport){setHasLocalDocumentOutput(false);return;}const hidden=new Set<HTMLElement>();const scan=()=>{let documentOutput=false;main.querySelectorAll<HTMLElement>("button,a,[role='button']").forEach(el=>{if(el.closest("[data-navilo-global-data-tools]"))return;if(isDocumentOutputAction(el)){documentOutput=true;return;}const label=labelOf(el);if(isTemplateAction(label)||isUploadAction(label)||isExportDuplicate(label)){el.style.setProperty("display","none","important");el.dataset.naviloDuplicateGlobalAction="true";hidden.add(el)}});setHasLocalDocumentOutput(prev=>prev===documentOutput?prev:documentOutput)};scan();const o=new MutationObserver(scan);o.observe(main,{childList:true,subtree:true});return()=>{o.disconnect();hidden.forEach(el=>{el.style.removeProperty("display");delete el.dataset.naviloDuplicateGlobalAction});setHasLocalDocumentOutput(false)}},[pathname,reportMode,masterMode,genericReport]);
  useEffect(()=>{
