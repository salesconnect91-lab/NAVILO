...
function isUploadAction(v:string){const x=normalize(v);return x==="import"||x.startsWith("import ")||x.includes("bulk upload")||x.includes("choose file")||x.includes("upload csv")||x.includes("upload excel")||x.includes("upload file")}
...
export default function UniversalDataTools(){
  ...
  useEffect(()=>{const main=document.querySelector<HTMLElement>("#navilo-main-content");if(!main||!standardPath)return;const hidden=new Set<HTMLElement>();const scan=()=>{let template=false,upload=false;main.querySelectorAll<HTMLElement>("button,a,[role='button']").forEach(el=>{if(el.closest("[data-navilo-global-data-tools]")||el.dataset.naviloKeepLocalAction==="true"||el.hasAttribute("data-direct-print"))return;const label=labelOf(el);if(isTemplateAction(label)){template=true;el.style.setProperty("display","none","important");hidden.add(el);return}if(isUploadAction(label)){upload=true;el.style.setProperty("display","none","important");hidden.add(el);return}if(isExportDuplicate(label)){el.style.setProperty("display","none","important");el.dataset.naviloDuplicateGlobalAction="true";hidden.add(el)}});setHasTemplate(prev=>prev===template?prev:template);setHasUpload(prev=>prev===upload?prev:upload)};scan();const o=new MutationObserver(scan);o.observe(main,{childList:true,subtree:true});return()=>{o.disconnect();hidden.forEach(el=>{el.style.removeProperty("display");delete el.dataset.naviloDuplicateGlobalAction});setHasTemplate(false);setHasUpload(false)}},[pathname,standardPath]);
  useEffect(()=>{
    ...
    const attach=()=>{
      ...
      const all=Array.from(main.querySelectorAll<HTMLElement>("button,a,[role='button']")).filter(el=>!el.closest("[data-navilo-global-data-tools]"));
