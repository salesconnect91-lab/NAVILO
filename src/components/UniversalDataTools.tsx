...
export default function UniversalDataTools(){
  ...
  const toolbar=<div className="navilo-report-toolbar" ref={ref} data-no-print data-no-export data-navilo-global-data-tools>
    ...
    {masterMode&&<button type="button" onClick={()=>window.dispatchEvent(new Event("navilo:master-refresh"))} className={base} title="Refresh master data"><RefreshCw className="h-4 w-4"/><span className="hidden xl:inline">Refresh</span></button>}
  </div>;
