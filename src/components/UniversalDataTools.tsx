...
function isMasterStandardPath(p:string){if(p.startsWith("/master-data/"))return true;return p==="/master-data"||p==="/sales/charges"||p.startsWith("/sales/charges/")}
...
export default function UniversalDataTools(){
  ...
  const reportMode=isReportPath(pathname),masterStandard=isMasterStandardPath(pathname),standardPath=isNaviloStandardPath(pathname),invoiceEditor=isInvoiceEditorPath(pathname),customizable=reportMode||masterStandard||hasCustomizableTable;
