...
export default function UniversalDataTools(){
  ...
  const[open,setOpen]=useState(false),[standardHost,setStandardHost]=useState<HTMLElement|null>(null),[hasCustomizableTable,setHasCustomizableTable]=useState(false),[hasLocalDocumentOutput,setHasLocalDocumentOutput]=useState(false),[selectedCount,setSelectedCount]=useState(0);
  ...
  useEffect(()=>{const sync=(event?:Event)=>{const detail=(event as CustomEvent<{selectedCount?:number}>)?.detail;const table=document.querySelector<HTMLElement>("#navilo-main-content [data-navilo-data-table]");setSelectedCount(Number(detail?.selectedCount??table?.dataset.naviloSelectedCount??0)||0)};sync();window.addEventListener("navilo:table-selection-changed",sync);return()=>window.removeEventListener("navilo:table-selection-changed",sync)},[pathname]);

  const exp=(t:"excel"|"csv"|"word",scope:"page"|"filtered"|"selected"="filtered")=>{if(!canExport)return;const root=currentExportRoot();if(!root)return;const title=currentPageTitle(),file=cleanTitle(title);const table=root.querySelector<HTMLElement>("[data-navilo-data-table]")??(root.matches("[data-navilo-data-table]")?root:null);const snapshots=table?.querySelector<HTMLElement>("[data-navilo-export-snapshots]");const snapshot=scope==="page"?null:snapshots?.querySelector<HTMLElement>(`[data-navilo-export-table="${scope}"]`);let exportRoot=root;let holder:HTMLDivElement|null=null;if(snapshot){holder=document.createElement("div");holder.dataset.reportContent="true";const clone=snapshot.cloneNode(true) as HTMLElement;clone.className="";clone.removeAttribute("aria-hidden");holder.appendChild(clone);document.body.appendChild(holder);exportRoot=holder}try{if(t==="excel")exportDomReportToExcel(file,exportRoot,title);if(t==="csv")exportDomReportToCSV(file,exportRoot,title);if(t==="word")exportDomReportToWord(file,exportRoot,title)}finally{holder?.remove();setOpen(false)}};
  ...
  const toolbar=<div className="navilo-report-toolbar" ref={ref} data-no-print data-no-export data-navilo-global-data-tools>
    ...
    {!invoiceEditor&&canExport&&<div className="relative"><button type="button" onClick={()=>setOpen(v=>!v)} className={base}><Download className="h-4 w-4"/><span>Export</span></button>{open&&<div className="absolute right-0 top-10 z-[70] w-48 rounded-lg border bg-white py-1 shadow-xl"><button type="button" onClick={()=>exp("excel","page")} className="flex w-full gap-2 px-3 py-2 text-xs"><Sheet className="h-4 w-4"/>Excel — Current Page</button><button type="button" onClick={()=>exp("excel")} className="flex w-full gap-2 px-3 py-2 text-xs"><Sheet className="h-4 w-4"/>Excel — Filtered</button><button type="button" onClick={()=>exp("csv","page")} className="flex w-full gap-2 px-3 py-2 text-xs"><Table2 className="h-4 w-4"/>CSV — Current Page</button><button type="button" onClick={()=>exp("csv")} className="flex w-full gap-2 px-3 py-2 text-xs"><Table2 className="h-4 w-4"/>CSV — Filtered</button><button type="button" onClick={()=>exp("word")} className="flex w-full gap-2 px-3 py-2 text-xs"><FileText className="h-4 w-4"/>Word — Filtered</button>{selectedCount>0&&<><div className="my-1 border-t"/><div className="px-3 py-1 text-[10px] font-semibold uppercase text-slate-400">{selectedCount} selected</div><button type="button" onClick={()=>exp("excel","selected")} className="flex w-full gap-2 px-3 py-2 text-xs"><Sheet className="h-4 w-4"/>Excel — Selected</button><button type="button" onClick={()=>exp("csv","selected")} className="flex w-full gap-2 px-3 py-2 text-xs"><Table2 className="h-4 w-4"/>CSV — Selected</button></>}<button type="button" onClick={print} className="flex w-full gap-2 px-3 py-2 text-xs"><FileType2 className="h-4 w-4"/>PDF / Print</button></div>}</div>}
    {!invoiceEditor&&canPrint&&!hasLocalDocumentOutput&&<button type="button" data-print-selector={reportSelector()} onClick={print} className={base}><Printer className="h-4 w-4"/><span>Print / PDF</span></button>}
    {masterMode&&<button type="button" onClick={()=>{window.dispatchEvent(new Event("navilo-master-data-changed"));window.dispatchEvent(new Event("navilo:master-refresh"))}} className={base} title="Refresh master data"><RefreshCw className="h-4 w-4"/><span>Refresh</span></button>}
  </div>;
