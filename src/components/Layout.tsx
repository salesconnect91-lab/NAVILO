...
const navigation:NavNode[]=[
  ...
  {key:"sales",label:"Sales / سیلز",icon:Lucide.ShoppingCart,module:"sales",children:[{key:"sales-invoices",to:"/sales",label:"Sales Invoices / سیلز انوائسز",end:true,module:"sales"},{key:"sales-workflow",to:"/sales/workflow",label:"Quotation to Dispatch / کوٹیشن تا ڈسپیچ",module:"sales"},{key:"sales-order-book",to:"/sales/order-book",label:"Sales Order Book / سیلز آرڈر بک",module:"sales"},{key:"sales-consolidated",to:"/sales/consolidated",label:"Consolidated Invoices / مشترکہ انوائسز",module:"sales",steelOnly:true}]},
  {key:"purchase",label:"Purchase / خریداری",icon:Lucide.Truck,module:"purchase",children:[{key:"purchase-invoices",to:"/purchase",label:"Purchase Invoices / خریداری انوائسز",end:true,module:"purchase"},{key:"purchase-workflow",to:"/purchase/workflow",label:"Requisition to GRN / ریکوزیشن تا وصولی",module:"purchase"},{key:"purchase-consolidated",to:"/purchase/consolidated",label:"Consolidated Purchase / مشترکہ خریداری",module:"purchase"},{key:"purchase-order-book",to:"/purchase/order-book",label:"Purchase Order Book / پرچیز آرڈر بک",module:"purchase"}]},
...
function shellLabel(label:string):string{
  // Keep English as the canonical shell source. GlobalLanguageRuntime translates
...
export default function Layout({children}:{children:ReactNode}){
  ...
  const[theme,setTheme]=useState<"light"|"dark">(()=>localStorage.getItem("navilo-theme")==="dark"?"dark":"light");
  const[density,setDensity]=useState<"compact"|"comfortable">(()=>localStorage.getItem("navilo-density")==="comfortable"?"comfortable":"compact");
  useEffect(()=>{localStorage.setItem("navilo-sidebar-collapsed",String(collapsed));},[collapsed]);
  useEffect(()=>{localStorage.setItem("navilo-theme",theme);document.documentElement.dataset.theme=theme;document.documentElement.classList.toggle("dark",theme==="dark");},[theme]);
  useEffect(()=>{localStorage.setItem("navilo-density",density);document.documentElement.dataset.density=density;},[density]);
  ...
  const render=(n:NavNode,d=0):ReactNode=>{
    ...
    if(has)return <div key={n.key}><button type="button" onClick={()=>setOpen(v=>({...v,[n.key]:!expanded}))} className={`navilo-nav-group flex min-h-9 w-full items-center gap-2 rounded-lg px-2 py-2 text-left transition-all duration-150 ${d===0?"text-[13px] font-bold":"text-[12.5px] font-semibold"} ${active?"is-active bg-white/[0.10] text-white":"text-slate-400 hover:bg-white/[0.06] hover:text-slate-100"}`}>{Icon&&<Icon className="h-4 w-4"/>}<span className="min-w-0 flex-1 truncate">{shellLabel(n.label)}</span><Lucide.ChevronDown className={`h-3.5 w-3.5 ${expanded?"rotate-180":""}`}/></button>{expanded&&!collapsed&&<div className={`${d===0?"ml-4":"ml-3"} mt-1 border-l border-white/10 pl-2`}>{n.children?.map(c=>render(c,d+1))}</div>}</div>;
    if(!n.to)return null;
    return <NavLink key={n.key} to={n.to} end={n.end} onClick={()=>setMobileOpen(false)} className={({isActive})=>`navilo-nav-link flex min-h-8 items-center gap-2 rounded-lg px-2 py-1.5 text-[12.5px] font-semibold transition-all duration-150 ${isActive?"is-active bg-blue-600 text-white shadow-sm shadow-blue-950/20":"text-slate-400 hover:translate-x-0.5 hover:bg-white/[0.06] hover:text-slate-100"}`}>{Icon&&<Icon className="h-4 w-4"/>}{!collapsed&&<span className="truncate">{shellLabel(n.label)}</span>}</NavLink>;
  };
  ...
  return <div className="erp-shell min-h-screen bg-[#f6f7f9] text-slate-900">
    ...
    <aside className={`navilo-sidebar fixed inset-y-0 left-0 z-50 flex w-[252px] flex-col border-r border-slate-800/80 bg-slate-950 text-slate-300 shadow-2xl shadow-slate-950/10 transition-all duration-200 ${side} ${mobileOpen?"translate-x-0":"-translate-x-full lg:translate-x-0"}`}>
      ...
      <div className="border-t border-white/10 p-2">{!collapsed&&<div className="mb-2 rounded-md bg-white/[0.03] px-2 py-2"><div className="text-[11px] uppercase text-slate-600">Active Company</div><div className="truncate text-[12px] font-bold text-slate-300">{location.pathname.startsWith("/owner")?"Owner Workspace":activeCompany?.company_name??"No company"}</div><div className="text-[11px] text-slate-500">{user?.email??"Signed in"} · {isPlatformOwner?"Platform Owner":roleLabel(role)}</div></div>}<button onClick={async()=>{await signOut();navigate("/login")}} className="flex h-9 w-full items-center justify-center gap-2 text-[12px] text-slate-500"><Lucide.LogOut className="h-4 w-4"/>{!collapsed&&"Sign out"}</button></div>
    </aside>
    <div className={`min-h-screen transition-all ${offset}`}>
      <header className="navilo-topbar sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="flex min-h-16 items-center gap-3 px-4 py-2 lg:px-5"><button onClick={()=>setMobileOpen(true)} className="lg:hidden"><Lucide.Menu/></button><button onClick={()=>setCollapsed(v=>!v)} className="hidden h-9 w-9 items-center justify-center rounded-md border lg:flex"><Lucide.PanelLeftClose className={`h-4 w-4 ${collapsed?"rotate-180":""}`}/></button>{location.pathname!=="/"&&<button onClick={()=>navigate(-1)} className="hidden h-9 items-center gap-1 rounded-md border border-blue-300 bg-blue-50 px-3 text-[13px] font-black text-blue-800 sm:inline-flex"><Lucide.ArrowLeft className="h-4 w-4"/>Back</button>}<div className="min-w-0 flex-1"><div className="truncate text-[16px] font-black text-slate-950">{pageTitle}</div></div><UniversalDataTools/><div className="hidden items-center gap-1 sm:flex"><button type="button" onClick={()=>setDensity(v=>v==="compact"?"comfortable":"compact")} title="Toggle layout density" aria-label="Toggle layout density" className="flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-50">{density==="compact"?<Lucide.Rows3 className="h-4 w-4"/>:<Lucide.ListFilter className="h-4 w-4"/>}</button><button type="button" onClick={()=>setTheme(v=>v==="light"?"dark":"light")} title="Toggle light or dark theme" aria-label="Toggle light or dark theme" className="flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-50">{theme==="light"?<Lucide.Moon className="h-4 w-4"/>:<Lucide.Sun className="h-4 w-4"/>}</button></div><div className="hidden text-right lg:block"><div className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{location.pathname.startsWith("/owner")?"Platform Workspace":"Active Company"}</div><div className="max-w-[220px] truncate text-[12px] font-black text-slate-800">{location.pathname.startsWith("/owner")?(branding.show_branding&&branding.erp_name?`${branding.erp_name} Platform`:"Owner Platform"):activeCompany?.company_name??"No company selected"}</div></div></div></header>
